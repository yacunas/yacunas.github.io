...
export default function About() {
  return (
    <section id="about" className="section-cv mx-auto max-w-5xl scroll-mt-24 px-5 py-20 sm:px-6 sm:py-32">
      <Reveal>
        <p className="font-mono text-sm text-amber-400">01 — About</p>
      ...
      <Reveal delay={0.1}>
        ...
        <p className="mt-4 inline-flex items-center gap-2 text-sm text-slate-400">
          <MapPin size={16} className="text-amber-400" /> {profile.location}
        </p>
      </Reveal>
      ...
      <div className="mt-12 grid grid-cols-2 gap-4 sm:grid-cols-4">
        {stats.map((s, i) => (
          ...
          >
            <div className="bg-gradient-to-r from-amber-300 to-orange-400 bg-clip-text text-3xl font-extrabold text-transparent sm:text-4xl">
              {s.value}
            </div>
      ...
      <Reveal delay={0.1}>
        <div className="mt-10 flex items-start gap-4 rounded-2xl border border-white/10 bg-white/[0.03] p-5">
          <GraduationCap className="mt-0.5 shrink-0 text-amber-400" />
          <div>
